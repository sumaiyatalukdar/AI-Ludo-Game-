...
class Ludo:
    def __init__(self, root,six_side_block,five_side_block,four_side_block,three_side_block,two_side_block,one_side_block):
        ...
        # Global ESC to prompt quit to main menu
        try:
        ...
        try:
            self.window.bind_class('Button', '<Button-1>', lambda e: self.play_click_sound(), add='+')
        except Exception:
            pass
        ...
        # Player display names (defaults); index: 0=red,1=sky_blue,2=yellow,3=green
        self.player_names = {
            0: "Player 1",
            1: "Player 2",
            2: "Player 3",
    ...
    def play_dice_sound(self):
        """Play dice roll sound non-blocking if possible."""
        def _play():
            try:
                # Prefer playsound for mp3 support
                try:
                    from playsound import playsound
                    playsound("sounds/dice_roll.mp3", block=False)
                    return
                except Exception:
                    pass
                # Fallbacks: simpleaudio/winsound (may not support mp3)
                try:
                    import simpleaudio  # type: ignore
                    # simpleaudio does not support mp3; skip silently
                except Exception:
                    pass
            except Exception:
                pass
    ...
    def play_move_sound(self):
        """Play coin move sound (wav) non-blocking."""
        def _play():
            try:
                # Try playsound (supports wav)
                try:
                    from playsound import playsound
                    playsound("sounds/Jump.wav", block=False)
                    return
                except Exception:
                    pass
                # Windows fallback
    ...
    def play_click_sound(self):
        """Play UI click sound non-blocking on button press."""
        def _play():
            try:
                try:
                    from playsound import playsound
    ...
    def take_initial_control(self):
        ...
        def filtering():# Total player input value filtering
            ...
            if response_take:
                ...
                # Ask names for selected number of players (after selecting Play With Friends)
                try:
                    self._ask_player_names(num_players=int(take_entry.get()), vs_computer=False)
                except Exception:
                    pass
                self.make_command()
                top.destroy()
        ...
        def operate(ind):
            if ind:
                ...
                for player_index in range(2):
                    self.total_people_play.append(player_index)
                print(self.total_people_play)
                # Prompt names for vs-computer (after selecting Play With Computer)
                try:
                    self._ask_player_names(num_players=2, vs_computer=True)
                except Exception:
                    pass
        ...
        top.mainloop()
    def _ask_player_names(self, num_players=2, vs_computer=False):
        try:
            from tkinter import Toplevel, Label, Entry, Button
            name_win = Toplevel(self.window)
            name_win.title("Enter Player Names")
            name_win.geometry("360x" + str(120 + num_players*50))
            name_win.resizable(False, False)
            colors = ["Red", "Sky Blue", "Yellow", "Green"]
            indices = [0,1,2,3]
            # Defaults
            defaults = [self.player_names.get(i, f"Player {i+1}") for i in indices]
            if vs_computer:
                defaults[0] = "Computer (Red)"
                defaults[1] = "You (Sky Blue)"
            Label(name_win, text="Enter names:", font=("Arial", 12, "bold")).pack(pady=8)
            entries = []
            for i in range(num_players):
                frame = self.make_canvas.create_rectangle  # placeholder to avoid linter on unused import
                lbl = Label(name_win, text=f"{colors[i]}", width=12, anchor="w")
                lbl.place(x=20, y=40 + i*40)
                ent = Entry(name_win, width=22)
                ent.insert(0, defaults[i])
                ent.place(x=130, y=40 + i*40)
                entries.append(ent)
            def apply_and_close():
                for i in range(num_players):
                    text = entries[i].get().strip() or f"Player {i+1}"
                ...
                try:
                    ...
                    pass
                name_win.destroy()
            Button(name_win, text="Apply", command=apply_and_close).place(x=150, y=50 + num_players*40)
            name_win.grab_set()
            name_win.focus_set()
        except Exception:
            pass

    def _apply_player_names_to_labels(self):
        # Rename the four Player labels on the board if present
        try:
            mapping = {
                "Player 1": self.player_names.get(0, "Player 1"),
                "Player 2": self.player_names.get(1, "Player 2"),
                "Player 3": self.player_names.get(2, "Player 3"),
                "Player 4": self.player_names.get(3, "Player 4"),
            }
            for child in self.make_canvas.winfo_children():
                try:
                    if getattr(child, "cget", None) is None:
                        continue
                    txt = child.cget("text")
                    if txt in mapping:
    ...
    def _handle_escape(self):
        try:
            from tkinter import messagebox
            if messagebox.askyesno("Quit Game", "Do you want to quit and return to main menu?"):
                try:
                ...
                # Return to main menu
                try:
                    from ludo_frontend import start_main_menu
                    start_main_menu()
                except Exception:
                    pass
        except Exception:
    ...
    def robo_judge(self, ind="give"):
        if ind == "give":# For give the value
            # Heuristic move selection (Expectimax-lite): try deterministic best move first
            try:
                legal_moves = self._legal_moves_red()
                if legal_moves:
                    best_move = self._choose_best_red_move(legal_moves)
                    # Track store if bringing a coin out
                    if self.red_coin_position[best_move-1] == -1 and self.move_red_counter == 6:
                        if best_move not in self.robo_store:
                            self.robo_store.append(best_move)
                    self.main_controller("red", best_move)
                    return
            except Exception as _:
                # Fall back to original logic below if heuristic fails
                pass
    ...
    def _opponent_positions_for_red(self):
        # In computer vs human mode, opponent is sky_blue
        return [pos for pos in self.sky_blue_coin_position if pos > -1 and pos < 106]

    def _is_entry_allowed_red(self, coin_index_one_based):
        # Red can enter from base only when dice is 6 and coin is at -1
        return self.red_coin_position[coin_index_one_based-1] == -1 and self.move_red_counter == 6

    def _legal_moves_red(self):
        moves = []
        for coin_idx in range(1, 5):
            pos = self.red_coin_position[coin_idx-1]
            if pos == -1:
                if self.move_red_counter == 6:
                    moves.append(coin_idx)
                continue
            # Already on track or home path
            if pos >= 100:
                if pos + self.move_red_counter <= 106:
                    moves.append(coin_idx)
            else:
                if pos + self.move_red_counter <= 106:
                    moves.append(coin_idx)
    ...
    def _evaluate_red_move(self, coin_index_one_based):
        # Higher is better
        dice = self.move_red_counter
        pos = self.red_coin_position[coin_index_one_based-1]
        new_pos = None
        # Compute resulting position without side-effects
        if pos == -1 and dice == 6:
            new_pos = 1
        elif pos >= 100:
            if pos + dice <= 106:
            ...
            if pos + dice <= 106:
                # Enter home path transition will be handled in UI; treat as linear progress
                if pos == 51:
                    new_pos = 100 if dice > 0 else 51
                else:
                    new_pos = pos + dice
                # Correct wrap-around at 53 -> 1
                if new_pos == 53:
                    new_pos = 1
            else:
                return float('-inf')
        ...
        # Base progress
        if pos == -1:
            score += 5.0  # entering the board
        else:
            score += (new_pos - pos) * 0.5

        # Reaching or progressing in home path
        if new_pos >= 100:
            score += 8.0
            if new_pos == 106:
                score += 20.0
        ...
        # Capture opponent
        opps = self._opponent_positions_for_red()
        if new_pos < 100 and new_pos in opps and new_pos not in self._safe_squares():
            score += 10.0

        # Penalize moving off a safe square into danger
        if pos in self._safe_squares() and new_pos not in self._safe_squares():
            score -= 2.0

        # Exposure penalty: if any opponent could capture us next turn (1-6 ahead)
        if new_pos < 100 and new_pos not in self._safe_squares():
            threat = False
            for o in opps:
                if o >= 100:
                    continue
                # Opp moves clockwise; if opponent can reach new_pos in 1..6 steps (with wrap at 53->1)
        ...
        # Prefer advancing the furthest piece when ties
        max_current = max([p for p in self.red_coin_position if p > -1] + [0])
        if pos == max_current:
            score += 1.0

        return score

    def _choose_best_red_move(self, legal_moves):
        best_score = float('-inf')
        best_move = legal_moves[0]
        for mv in legal_moves:
            s = self._evaluate_red_move(mv)
            if s > best_score:
                best_score = s
                best_move = mv
        return best_move
